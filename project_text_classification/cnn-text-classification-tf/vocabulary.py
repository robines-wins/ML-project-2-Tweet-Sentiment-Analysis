...
class Vocabulary(object):
	...
	def fit(self,strlist):
		...
		for s in strlist:
			for w in s.split():
				# take care only of words not already processed
				if w not in self.vocab:
					# Store the index of the word (in the embedding) to the vocabulary
					self.vocab[w]=self.vocsize
					self.vocsize += 1

					# Use the protocol explained above to assign vectors to words and put them in the embedding
		...
		for s in strlist:
			...
			for idx,w in enumerate(s.split()):
				if idx >= self.maxlength:
					# If the sequence is too long (which should not happen in our case, then we just cut it rather than crashing)
					break
				if w in self.vocab:
